...
// Delete all articles (keep the book with id=1)
console.log('Removing old articles...');
db.prepare('DELETE FROM publications WHERE category = ?').run('article');
...
console.log('Adding updated articles to database...\n');
...
const all = db.prepare('SELECT id, title, category, author, pdf_file FROM publications ORDER BY id').all();
console.table(all);
